#!/usr/bin/python
# -*- codding: utf-8 -*-
import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))
from common.execute_command import write_two_parameter

# url : https://awscli.amazonaws.com/v2/documentation/api/latest/reference/workmail/delete-access-control-rule.html
if __name__ == '__main__':
    """
	list-access-control-rules : https://awscli.amazonaws.com/v2/documentation/api/latest/reference/workmail/list-access-control-rules.html
	put-access-control-rule : https://awscli.amazonaws.com/v2/documentation/api/latest/reference/workmail/put-access-control-rule.html
    """

    parameter_display_string = """
    # organization-id : The identifier for the organization.
    # name : The name of the access control rule.
    """
    add_option_dict = {}
    add_option_dict["parameter_display_string"] = parameter_display_string
    # ex: add_option_dict["no_value_parameter_list"] = "--single-parameter"
    write_two_parameter("workmail", "delete-access-control-rule", "organization-id", "name", add_option_dict)
